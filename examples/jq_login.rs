//!
//! This example showcases a fully config-driven JSON-body login flow, used
//! by providers that authenticate via a single JSON POST request rather than
//! a standard OAuth2 grant, e.g.:
//!
//! ```sh
...
//! shape of request, so this example builds the request/response by hand
//! using the crate's public `HttpRequest`/`HttpResponse`/`SyncHttpClient`
//! types, the same way `letterboxd.rs` builds a custom signing client on top
//! of them.
//!
...
//!
//! All of these are merged into one filter output, e.g.
//! `{client_id: .client_id, client_secret: .client_secret, grant_type:
//! "client_credentials", scope: "read write", audience:
//! "https://api.example.com"}`. Only `client_id`/`client_secret` placement
//! in headers or HTTP Basic auth (as opposed to the body) is controlled by
//! [`CredentialPlacement`] instead of the filter, since that's outside the
//! JSON body `request_filter` produces.
//!
...
//! a system `jq` binary.
//!
//! [`JqLoginConfig`] and [`CredentialPlacement`] derive `Deserialize`, so the
//! whole shape - not just the credential values - can be supplied at
//! runtime as a JSON string, letting the same compiled binary target a
//! different provider without a rebuild. Set `JQ_LOGIN_CONFIG` to a JSON
//! document like:
//!
//! ```json
//! {
...
//! ```
//!
//! or, for a provider using HTTP Basic auth instead: `"credentials": "basic_auth"`.
//! If `JQ_LOGIN_CONFIG` isn't set, a built-in default config (matching the
//! sample request/response above) is used instead.
//!
...
//! cargo run --example jq_login --features reqwest-blocking
//! ```
...
use oauth2::{
    AccessToken, ClientId, ClientSecret, EmptyExtraTokenFields, HttpRequest, HttpResponse,
    RefreshToken, Scope, StandardTokenResponse, SyncHttpClient,
};
...
fn main() -> Result<(), anyhow::Error> {
    ...
    // provider is just a config string change, not a recompile. See the
    // module docs above for the JSON format.
...
/// The built-in config used when `JQ_LOGIN_CONFIG` isn't set, matching
/// the sample request/response in the module docs above.
...
impl JqLoginClient {
    ...
    fn prepare_request(&self) -> Result<HttpRequest, anyhow::Error> {
...
/// used by this example are expected to always produce exactly one object.
...
mod tests {
    ...
    fn supports_arbitrary_field_names_and_extra_request_fields() {
        ...
        let request = client.prepare_request().unwrap();
